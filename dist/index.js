...
if (process.env.NODE_ENV == "production" && _cluster2.default.isMaster) {
  for (var i = 0; i < numCPUs; ++i) {
    _cluster2.default.fork();
  }
} else {
  var workerId = process.env.HOSTNAME + _cluster2.default.worker;
  logger.info('the marker ', workerId);
  ...
  var onActionSub = function onActionSub(_ref) {
    ...
    return function (msg, cb) {
      ...
      pubClient.hset(config.siteMap(workerId), CustomerID, socket.id, function (err, msg) {
  ...
  var onDisconnect = function onDisconnect(_ref2) {
    ...
    return function () {
      ...
      pubClient.hdel(config.siteMap(workerId), CustomerID, function (err, msg) {
