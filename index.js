...
if(process.env.NODE_ENV == "production" && cluster.isMaster) {
  ...
  const workerId = process.env.HOSTNAME + cluster.worker
  ...
  const onActionSub = ({socket, CustomerID}) => (msg, cb) => {
    ...
    pubClient.hset(config.siteMap(workerId), CustomerID, socket.id, (err, msg)=>{
  ...
  const onDisconnect = ({socket}) => () => {
    ...
    pubClient.hdel(config.siteMap(workerId), CustomerID, (err, msg)=>{
